...
std::vector<int> voxel(IX*IY*IZ,1);

cudamodel::cudamodel(int resX, int resY, int resZ)
{
...
struct Projection
{
	...
	bool checkRange(double x, double y, double z)
	{
		...
		if (outOfRange(indX, height) || outOfRange(indY, width))
			return false;
		return m_image.at<uchar>(indX, indY) > m_threshold;
	}
};
	...
	, m_corrZ(resZ, 15, 30)
{
	if (resX > 100)
		...
		m_neiborSize = 1;
	m_surface = Voxel(m_corrX.m_resolution, Pixel(m_corrY.m_resolution, std::vector<bool>(m_corrZ.m_resolution, true)));
}
...
void Model::getModel()
{
	cudamodel cumodel(300, 300, 300);
	float m1[3][4];
	uchar* mat[20];
	std::vector<uchar> m2(prejectionCount*height*width);
	for(int n=0;n<prejectionCount;n++)
	{
		for(int i=0;i<3;i++)
		{
			for(int j=0;j<4;j++)
			{
			    m1[i][j]=m_projectionList[n].m_projMat(i,j);
			}
		}
		for(int i=0;i<height;i++)
		{
			...
			{
			    m2[n*width*height+i*width+j]=m_projectionList[n].m_image.at<uchar>(i,j);
			}
		}
		cumodel.m_projectionList.push_back(cudaProjection(m1));
		mat[n]=m2.data()+n*width*height;
	}
	cumodel.compute(voxel.data(),cumodel.m_projectionList.data(),mat);
}
...
void Model::getSurface()
{
	...
	for (int indexX = 0; indexX < m_corrX.m_resolution; indexX++)
		for (int indexY = 0; indexY < m_corrY.m_resolution; indexY++)
			for (int indexZ = 0; indexZ < m_corrZ.m_resolution; indexZ++)
			{
				if (!voxel[indexX*IY*IZ+indexY*IZ+indexZ])
				{
				...
				while(!ans&&i<6)
				{
					...
					iy=indexY + dy[i];
					iz=indexZ + dz[i];
					ans = ans || outOfRange(ix, iy, iz)|| !voxel[ix*IY*IZ+iy*IZ+iz];
					++i;
				}
		...
		for (int dY = -m_neiborSize; dY <= m_neiborSize; dY++)
			for (int dZ = -m_neiborSize; dZ <= m_neiborSize; dZ++)
			{
				...
				if (!outOfRange(neiborX, neiborY, neiborZ))
				{
					...
					else if (voxel[neiborX*IY*IZ+neiborY*IZ+neiborZ])
...
int main(int argc, char** argv)
{
	...
	// 得到Voxel模型
	model.getModel();
	std::cout << "get model time: " << (float(clock()-t) / CLOCKS_PER_SEC) << "seconds\n";
